...
var config = {
  ...
  output: {
    ...
    path: path.join(__dirname, 'azure/public'),
    //                  first thought
  ...
  devServer: {
    ...
    // client and server hosted on the same address (protocal+ domain +  port)
    // so we don't need proxy
    // proxy: { '/graphql': `http://relay-modern.azurewebsites.net` }
  }
}
